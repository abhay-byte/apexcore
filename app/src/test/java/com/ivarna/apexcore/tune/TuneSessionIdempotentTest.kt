...
class TuneSessionIdempotentTest {
    ...
    @Test
    fun testDoubleApplyForSessionDoesNotCorruptOriginalSnapshot() = runBlocking {
        ...
        assertEquals("0", snapshotStore.getOriginal(sconfigPath))

        // Second apply (e.g. redelivery)
        manager.applyForSession("com.test.game")
        assertEquals("13", fakeShell.pathValues[sconfigPath])
        assertEquals("0", snapshotStore.getOriginal(sconfigPath)) // Original remains "0"
    }
}
