...
class TuneMidSessionOnAppliesNowTest {
    ...
    @Test
    fun testMidSessionTurningOnBundleAppliesImmediatelyOnIO() = runBlocking {
        ...
        // Mid-session: turn ON CPU_FLOOR
        manager.setIntent(TuneId.CPU_FLOOR, TuneValue(on = true))
        delay(150) // Allow IO apply to finish

        // CPU_FLOOR must now be applied
